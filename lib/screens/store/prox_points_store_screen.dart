...
class _ProxPointsStoreScreenState extends State<ProxPointsStoreScreen> {
  ...
  Future<void> _openExample(
    ...
    await Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => sku.startsWith("cosmetic_")
            ? const CosmeticsScreen()
            : FeatureExampleScreen(title: "$title example"),
      ),
    );
  }
  ...
  Future<void> _loadCardsOnFile() async {
    ...
    final cached = await MonetizationService.instance.listPaymentMethodsCached(
      uid: uid,
    );
  ...
  Future<StorePurchaseResult> _verifyCardCheckoutForStore({
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Payment is still processing. Verify again shortly."),
        ),
      );
    }
    return const StorePurchaseResult(StorePurchaseStatus.unknownSku);
    ...
    try {
      ...
      if (checkoutUrl.isEmpty || sessionId.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("Checkout is not configured for this build yet."),
            ),
          );
        }
        return const StorePurchaseResult(StorePurchaseStatus.unknownSku);
  ...
  Future<void> _applyRuntimeEffectsForSku(String sku) async {
    ...
    if (sku == "biz_boost_visibility") {
      // ignore: discarded_futures
      PresenceWriter.instance.writeOneShot(
        reason: "store_visibility_boost_purchase",
      );
    }
  ...
  Future<void> _syncHighRadiusEntitlement() async {
    ...
    final unlocked = await MonetizationService.instance.isHighRadiusUnlocked(
      uid,
    );
  ...
  Future<void> _handleTap({
    ...
    if (!_implementedSkus.contains(sku) ||
        (requiresBusiness && !businessUnlocked)) {
      await _openExample(context, sku, title);
      return;
    }

    UiTelemetryService.instance.log("store_item_tap", meta: {"sku": sku});
    ...
    try {
      if (!requiresBusiness && _checkoutMode == _StoreCheckoutMode.savedCard) {
        ...
        result = await StorePurchaseService.instance
            .purchase(uid: uid, sku: sku, businessUnlocked: businessUnlocked)
            .timeout(
              timeout,
              onTimeout: () {
                purchaseTimedOut = true;
                return const StorePurchaseResult(
                  StorePurchaseStatus.unknownSku,
                );
              },
            );
      }
    ...
    switch (result.status) {
      case StorePurchaseStatus.purchased:
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Purchased $sku for ${result.pointsSpent} points."),
          ),
        );
        break;
      ...
      case StorePurchaseStatus.locked:
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "This item requires approved Pro access. You can explore its local example.",
            ),
          ),
        );
        break;
  ...
  Widget build(BuildContext context) {
    ...
    Widget tile({
      ...
      if (exampleOnly) {
        return Card(
          child: ListTile(
            ...
            title: Text(title),
            subtitle: Text(
              "$subtitle\nInteractive example · Free · No account changes",
            ),
            trailing: const Icon(Icons.play_circle_outline),
            onTap: () => _openExample(context, sku, title),
          ),
        );
      }
      final bool unlockedByBusinessGate =
          requiresBusiness && businessUnlocked && pointsCost == null;
      final bool effectiveOwned =
          owned || _ownedSkus.contains(sku) || unlockedByBusinessGate;
      final bool lockedByBusiness = requiresBusiness && !businessUnlocked;
      ...
      final String lockReason = lockedByBusiness
          ? "Pro access required; example available"
          : lockedByPoints
                    ...
                    Row(
                      children: [
                        ...
                        if (effectiveOwned)
                          ...
                          const TesterBadge(
                            label: "Buy",
                            semanticsLabel: "Available to buy",
                          ),
                      ],
                    ),
              ...
              if (_purchaseInFlightSku == sku)
                ...
                OutlinedButton(
                  onPressed: lockedByBusiness
                      ...
                      : null,
                  child: Text(lockedByBusiness ? "Example" : "Need pts"),
                ),
            ],
          ),
        ),
            ...
            return StreamBuilder<PointsMeta>(
              ...
              builder: (context, snap) {
                ...
                return FutureBuilder<BusinessGateState>(
                  future: BusinessModeEligibility.gateForUser(
                    uid: uid,
                    meta: meta,
                  ),
                  builder: (context, gateSnap) {
                    final gate =
                        gateSnap.data ??
                        BusinessModeEligibility.gateFromMeta(meta);
                      ...
                      body: ListView(
                        ...
                        children: [
                          ...
                          tile(
                            ...
                            subtitle:
                                "Distinctive frame styles for your profile.",
                            cost: "35 pts",
                            pointsCost: 35,
                          ...
                          tile(
                            ...
                            subtitle:
                                "Temporary spotlight in discovery surfaces.",
                            cost: "80 pts",
                            pointsCost: 80,

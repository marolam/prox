    ...
    if (!ProModePreviewAccess.instance.isAllowedForCurrentUser()) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content:
                  Text("Pro Mode preview is not available for this account.")),
        );
      }
      return;
    }
    ...
    if (!earned) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
                "Business Mode is locked. Complete progression requirements first."),
          ),
        );
      }
      return;
              ...
              Card(
                ...
                child: Column(
                  ...
                  children: [
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            simpleMode
                                ? "Keep your name, selfie, Searching For keywords, and Can Provide keywords accurate."
                                : "Update name, selfie, headline, keywords, and Business availability here.",
                            style: theme.textTheme.bodySmall
                    ...
                    const ProfileEditScreen(inline: true),
                  ],
                ),
              ),
              if (!simpleMode) ...[
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        if (isBusinessMode || canUseProMode)
                          FilledButton(
                            onPressed: () => isBusinessMode
                                ? _switchToParty(context)
                                : _switchToBusiness(context),
                            child: Text(isBusinessMode
                                ? "Switch to Personal"
                                : "Switch to Pro"),
                          ),
                      ],
                        ...
                        builder: (context, snap) {
                          ...
                          return StreamBuilder<PointsMeta>(
                            ...
                            builder: (context, pointsSnap) {
                              final points = pointsSnap.data ??
                                  PointsService.instance.peekMeta(uid);
                              final int level = progression?.level ??
                                  ProgressionService.instance
                                      .levelFromTotalPoints(points.totalPoints);
                              final int toNext = ProgressionService.instance
                                  .pointsToNextLevel(points.totalPoints);
                              final int ageDays =
                                  progression?.accountAgeDays ?? 0;

                              ProxFeatureUnlock? nextFeature;
                              for (final feature in ProxFeatureUnlock.values) {
                                final unlocked =
                                    progression?.has(feature) ?? false;
                                if (!unlocked) {
                                  nextFeature = feature;
                                  break;
                                }
                              ...
                              final String nextLabel = nextFeature == null
                                  ...
                                  : ProgressionService.instance
                                      .requirementLabel(nextFeature);
                              final String nextReason = (nextFeature == null ||
                                      progression == null)
                                  ? ""
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              Switch(
                                ...
                                onChanged: (v) => UserSettingsService.instance
                                    .setReferralSignalEnabled(v),
                              ),
                            ],
                          ),
                          const SizedBox(height: 4),
                          ...
                          StreamBuilder<UserProfile?>(
                            stream:
                                UserProfileService.instance.watchProfile(uid),
                            builder: (context, snap) {
                              final me = snap.data;
                              final refUid = (me?.referrerUid ?? "").trim();
                              if (refUid.isEmpty ||
                                  !settings.referralSignalEnabled) {
                                return Container(
                                  width: double.infinity,
                                  padding: const EdgeInsets.all(12),
                                  decoration: BoxDecoration(
                              ...
                              return FutureBuilder<UserProfile?>(
                                future: UserProfileService.instance
                                    .getProfileOnce(refUid),
                                builder: (context, refSnap) {
                                  final ref = refSnap.data;
                                  final name =
                                      (ref?.displayName ?? "a friend").trim();
                          ...
                          FutureBuilder<ModeUnlockState?>(
                            ...
                            builder: (context, unlockSnap) {
                              ...
                              return FutureBuilder<bool>(
                                future: MonetizationService.instance
                                    .isBusinessUnlocked(uid),
                                builder: (context, paidSnap) {
                                  ...
                                  if (paid) {
                                    status =
                                        "Status: active access (toggle in Profile/Settings)";
                                  } else if (earned) {
                                    status =
                                        "Status: unlocked to buy (activation available)";
                                  } else {
                                    status =
                                        "Status: locked (earn unlock first)";
                                  }
                                    ...
                                    children: [
                                      ...
                                      SizedBox(
                                        ...
                                        child: FilledButton.icon(
                                          ...
                                          label: Text(paid
                                              ? "Manage Business"
                                              : "Open Business paywall"),
                                          onPressed: () =>
                                              MonetizationEntryService.instance

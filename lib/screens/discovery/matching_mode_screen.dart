...
class _MatchingModeScreenState extends State<MatchingModeScreen> {
  ...
  Future<void> _refreshGuidanceContext() async {
    ...
    final entitlements = await MonetizationService.instance.getEntitlementsMap(uid: uid);
    ...
    UserSettingsService.instance.setSingleKeywordMatchUnlocked(singleKeywordUnlocked);
  ...
  int _estimateKeywordCount(UserProfile? profile) {
    ...
    final items = <String>{
      ...
      ...profile.canProvide,
      if ((profile.searching ?? "").trim().isNotEmpty) profile.searching!.trim(),
      if ((profile.providing ?? "").trim().isNotEmpty) profile.providing!.trim(),
    };
  ...
  List<String> _buildGuidance(double maxAllowedRadius) {
    ...
    if (maxAllowedRadius <= 15.0 &&
      ...
      suggestions.add("Unlock High Radius in the Prox Store to extend Business Passive search up to 30 mi.");
    }

    if (_radiusMiles > 15.0 && _keywordCount < 3) {
      suggestions.add("Add at least 3 clear search/provide keywords for better far-radius match quality.");
    }

    if (_kind == MatchingModeKind.travel) {
      suggestions.add("Travel mode is strict. If results are sparse, switch to Normal Passive for broader coverage.");
    }

    if (_radiusMiles > 15.0) {
      suggestions.add("At very high radius, far profiles are filtered to keyword overlaps to reduce low-intent noise.");
    }

    if (!_singleKeywordUnlocked || !_reciprocalUnlocked || !_keywordChainUnlocked) {
      suggestions.add("Prox Store unlocks additional keyword modes: Single Keyword, Reciprocal Opposite, and Keyword Chain.");
    }
  ...
  void _commitAndClose() {
    try {
      ...
      final safeRadius = _radiusMiles
          .clamp(MatchDiscoverySettings.minRadiusMiles, MatchDiscoverySettings.extendedMaxRadiusMiles)
          .toDouble();
              ...
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Slider(
                        ...
                        divisions: ((maxAllowedRadius - MatchDiscoverySettings.minRadiusMiles) * 2)
                            .round()
                            .clamp(1, 100),
                ...
                if (!widget.focusRadius) ...[
                  ...
                  GridView.count(
                    ...
                    children: [
                      ...
                      ProxGlassCard(
                        onTap: () => _setKind(MatchingModeKind.treasureHunt),
                        highlight: green,
                        glow: green,
                        padding: const EdgeInsets.all(14),
                        child: _ModeCardInner(
                          ...
                          selected: _kind == MatchingModeKind.treasureHunt,
                          accent: green,
                        ),
                      ),
                      ProxGlassCard(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SegmentedButton<NormalMatchMode>(
                            segments: const [
                              ButtonSegment(value: NormalMatchMode.passive, label: Text("Passive")),
                              ButtonSegment(value: NormalMatchMode.active, label: Text("Active")),
                            ],
                            selected: <NormalMatchMode>{_normalMode},
                            onSelectionChanged: _activeLocked
                                ? null
                                : (next) {
                                    if (next.isNotEmpty) _setNormalMode(next.first);
                                  },
                          ),
                          const SizedBox(height: 8),
                          Text(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Slider(
                            ...
                            divisions: 19,
                            onChanged: (v) => setState(() => _treasureRadius = v),
                          ),
                        ],
                      ),
                    ),
                ...
                ProxGlassButton(
                  ...
                  highlight: _kind == MatchingModeKind.travel
                      ? orange
                      : (_kind == MatchingModeKind.treasureHunt ? green : blue),
                ),
